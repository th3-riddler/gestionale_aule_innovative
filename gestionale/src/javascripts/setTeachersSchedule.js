function getPos() {
  ...
  if (cell.classList.contains("selected")) {
    // if the cell is already selected, i'm trying to deselect it
    cell.classList.remove("selected");
    cell.firstElementChild.tagName === "BUTTON"
      ? cell.firstElementChild.classList.add("btn-disabled")
      : null;
    ...
    if (cell.firstElementChild.tagName !== "BUTTON") {
      cell.firstElementChild.lastElementChild.firstElementChild.classList.add(
        "btn-accent"
      );
      cell.firstElementChild.lastElementChild.firstElementChild.classList.remove(
        "btn-info"
      );
    }
    ...
    form.removeChild(
      form.querySelector(
        'input[name="hour[]"][value="' + cell.id.split("")[0] + '"]'
      )
    );
    form.removeChild(
      form.querySelector(
        'input[name="weekday[]"][value="' +
          document.querySelectorAll("th")[parseInt(cell.id.split("")[1])]
            .textContent +
          '"]'
      )
    );
  ...
  if (cell.firstElementChild.classList.contains("btn-disabled")) {
    cell.firstElementChild.classList.remove("btn-disabled");
  }
  ...
  if (cell.classList.contains("selected")) {
    cell.classList.remove("selected");
    let inputHour = form.querySelector(
      'input[name="hour[]"][value="' + hour + '"]'
    );
    let inputDay = form.querySelector(
      'input[name="weekday[]"][value="' + weekday + '"]'
    );
...
function viewOptions() {
  ...
  customizationDiv.innerHTML = `<div class='btn btn-square btn-outline ${
    this.classList.contains("btn-accent") ? "btn-info" : "btn-accent"
  } border-2 btn-md'>
                                  ...
                                  <a href="../API/deleteTeacherSchedule.php?hour=${
    ...
    document
      .getElementsByClassName("tabs-boxed")[0]
      .querySelector(".tab-active").textContent
  }" class='btn btn-square btn-outline btn-error border-2 btn-md'>
                                    <div>
